...
export default function Approve() {
  ...
  // Calculate total ETH and check for sufficient balance
  useEffect(() => {
    const total = validAddresses.reduce((sum, entry) => sum + parseFloat(entry.amount || 0), 0);
    setTotalETH(total);
  ...
  const proceedToMultisend = () => {
    if (!insufficientETH && validAddresses.length > 0) {
      setStatus("Approve");
      setTimeout(() => {
        setStatus("Multisend");
        router.push({
          ...
          <div className="border border-blue-900 rounded-xl">
            <div className="bg-gradient-to-r from-[#1e293b] to-[#0F123D] bg-opacity-80 p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6 rounded-xl">
                <BalanceBox label="Total ETH to send" value={`${totalETH} ETH`} />
                <BalanceBox label="Your ETH balance" value={`${accountETH} ETH`} />
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-[#1e293b] to-[#0F123D] bg-opacity-80 p-6 rounded-lg text-xs text-blue-400">
              ...
              <AddressList addresses={validAddresses} highlight="blue" />

              <h3 className="text-lg font-semibold mt-6 mb-4">Invalid Addresses</h3>
              <AddressList addresses={invalidAddresses} highlight="red" emptyMessage="All addresses are valid!" />
              ...
              <div className="mt-6 space-y-4">
                ...
                <button
                  ...
                  className={`w-full ${
                    !insufficientETH && validAddresses.length > 0
...
function AddressList({ addresses, highlight, emptyMessage }) {
  if (addresses.length === 0 && emptyMessage) {
    return <p className={`text-${highlight}-500`}>{emptyMessage}</p>;
  }
  return (
    <div className={`space-y-2 text-${highlight}-400`}>
